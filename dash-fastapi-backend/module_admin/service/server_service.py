        ...
        try:
            ...
            for i in io:
                try:
                    ...
                    print(f"Device: {i.device}, Total: {o.total}, Used: {o.used}, Free: {o.free}")
                except PermissionError:
                    print(f"Permission denied to access {i.device}")
                    continue
                except FileNotFoundError:
                    print(f"Device {i.device} not found")
                    continue
                except Exception as e:
                    print(f"Error accessing {i.device}: {str(e)}")
                    continue
                disk_data = SysFiles(
                    dir_name=i.device,
            ...
            print('get_server_monitor_info error:', traceback.format_exc())
        return result
